...
class BacktestEngine:
    ...
    def _open_trade(self, signal: TradeSignal, candle: Candle,
        ...
        adjusted_leverage = cfg.leverage
        
        if cfg.leverage in LEVERAGE_POSITION_LIMITS:
            max_position = LEVERAGE_POSITION_LIMITS[cfg.leverage]
            if position_size > max_position:
                # 查找下一个杠杆层级（更低的杠杆，更大的仓位限制）
                sorted_leverages = sorted(LEVERAGE_POSITION_LIMITS.keys(), reverse=True)
                current_leverage_idx = sorted_leverages.index(cfg.leverage)
                
                # 从当前杠杆开始，向下查找更低的杠杆
                for i in range(current_leverage_idx + 1, len(sorted_leverages)):
                    next_leverage = sorted_leverages[i]
                    next_max_position = LEVERAGE_POSITION_LIMITS[next_leverage]
                    
                    # 检查使用这个杠杆是否可以容纳当前仓位
                    if position_size <= next_max_position:
                        adjusted_leverage = next_leverage
                        log.info(f"仓位 ${position_size:,.2f} 超过 {cfg.leverage}x 限制 ${max_position:,.2f}，"
                                f"自动调整杠杆到 {adjusted_leverage}x (最大限制: ${next_max_position:,.2f})")
                        break
                
                # 如果找到合适的杠杆，重新计算保证金
                if adjusted_leverage != cfg.leverage:
                    margin = position_size / adjusted_leverage
                    # 更新配置中的杠杆，用于后续计算
                    cfg.leverage = adjusted_leverage
                else:
                    # 没有找到合适的杠杆，调整仓位大小到当前杠杆的最大限制
            ...
            if position_size > max_position:
                ...
                for leverage in sorted_leverages[1:]:  # 跳过150x
                    max_pos = LEVERAGE_POSITION_LIMITS[leverage]
                    if position_size <= max_pos:
                        adjusted_leverage = leverage
                        log.info(f"仓位 ${position_size:,.2f} 超过 150x 限制 ${max_position:,.2f}，"
                                f"自动调整杠杆到 {adjusted_leverage}x (最大限制: ${max_pos:,.2f})")
                        break
